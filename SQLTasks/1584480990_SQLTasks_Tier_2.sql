...
FROM
    `Members` m
        left JOIN
(select b.memid,f.name,IF(b.memid = 0,
        f.guestcost,
        f.membercost) AS cost from 
    `Bookings` b
        LEFT JOIN
    `Facilities` f ON f.facid = b.facid
WHERE
    DATE(starttime) = '2012-09-14') as bf on bf.memid = m.memid where bf.cost > 30
